// Journal — a mid-century biomedical-journal house format: a small-caps masthead,
// a full-width title block and structured abstract, then a justified TWO-COLUMN
// body. Serif text in New Computer Modern, sans masthead and section rubrics in
// Inter (both bundled), a deep-red accent. Canonical ABI.
//
// Beyond the base ABI it also reads four optional masthead fields — `journal`,
// `articletype`, `affiliation`, `received` — declared as named params BEFORE
// `body, ..extra`. A document that supplies them (e.g. a journal-styled seed)
// gets a populated masthead; any other doc leaves them `none` and the masthead
// falls back to neutral defaults. Extra named args from other styles' covers land
// in `..extra` and are ignored.
...
#let doc(
  ...
  set page(
    ...
    numbering: "1",
    number-align: center,
    // A running head from page 2 on, kept off the title page so the masthead
    // stands alone. Generic: the journal name (or the title) on the left, the
    // author on the right.
    header: context {
      if counter(page).get().first() > 1 {
        ...
        grid(
          ...
          [#if journal != none { journal } else { title }],
          align(right)[#author],
        )
        v(0.2em)
  ...
  block(width: 100%)[
    #grid(
      ...
      text(font: "Inter", size: 12.5pt, weight: 800, fill: accent, tracking: 0.03em)[
        #if journal != none { smallcaps(journal) }
      ],
  ...
  if date != none {
    v(0.3em)
    text(font: "Inter", size: 7pt, fill: ink-soft, tracking: 0.04em)[#upper(date)]
  } else if received != none {
    v(0.3em)
