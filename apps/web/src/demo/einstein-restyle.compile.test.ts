...
import { DEMO_FILES, DEMO_MAIN } from "./einstein-1905.js";
import { BUILT_IN_STYLES } from "../styles-library/index.js";

// Real typst.ts compilation, in Node — proves the Einstein body survives a REAL
// restyle. The live demo (#20) now ships a conforming `/style.typ`, so the
// Style Library can swap it in place. This gate takes the live `DEMO_FILES`
// tree, REPLACES `/style.typ` with EACH bundled style (academic / modern /
// minimal), and asserts every restyled tree still compiles offline with zero
// error diagnostics — the same offline WASM the app ships, built-in fonts only.
// (The styles drop the demo-specific `eyebrow`/`subtitle` cover chrome into
// their `..extra` sink and ignore it; the body is what must keep compiling.)
...
/** The live demo tree with `/style.typ` swapped for `styleText`. */
function treeWithStyle(styleText: string): { path: string; text: string }[] {
  return DEMO_FILES.map((f) => (f.path === "/style.typ" ? { ...f, text: styleText } : f));
}
...
describe("Einstein demo survives a real restyle (#20 / styles Phase 1.5)", () => {
  for (const style of BUILT_IN_STYLES) {
    ...
    it(`restyled with "${style.manifest.id}" compiles offline with zero errors`, async () => {
      const res = await engine.check({ kind: "project", main: DEMO_MAIN, files });
