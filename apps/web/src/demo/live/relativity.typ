// Paper III — "On the electrodynamics of moving bodies" (30 June 1905), with
// the mass–energy result of the September addendum folded in as its closing
// section. Cross-references the light-cone figure in /spacetime.typ via
// @lightcone (cross-file labels resolve through the #include chain).
...
Carrying the same postulates into the dynamics of a radiating body shows that
to emit energy is to lose mass. Let a body at rest give off the energy $L$ as
radiation, half to each of two opposite directions; reckoned from a moving
frame, the bookkeeping balances only if the body's mass has diminished by
$L \/ c^2$. The inertia of a body is a measure of its energy-content, the two
bound by a single constant:

$ E = m c^2 $

Because $c^2$ is enormous, a vanishing mass conceals a vast energy --- the
seed, decades hence, of the reactor and the star. Bodies whose energy-content
is variable to a high degree --- the salts of radium, say --- may one day put
the relation to the test.
